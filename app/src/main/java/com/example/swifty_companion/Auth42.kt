...
class Auth {
    ...
    suspend fun findPeerRequest(login: String?): List<UserInfo>? {
        ...
        try {
            client.newCall(request).execute().use {
                ...
                val itemType = object : TypeToken<List<UserInfo>>() {}.type
                users = Gson().fromJson<List<UserInfo>>(it.body!!.string(), itemType)
